...
export default function InvitacionesClient({
  ...
  async function fetchPage(t: 'todas' | 'aceptada' | 'pendiente', p: number, search?: string) {
    return getInvitacionesAction({
      ...
      busqueda: search ?? busquedaRef.current,
    });
  }
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="relative">
          <input
            ...
            placeholder="Buscar por comerciante, WhatsApp o distribuidor..."
          ...
          {total > 0 && (
            <div className="flex items-center justify-between p-4 border-t border-admin-border">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                Pag {page} de {totalPages} · {total} invitaciones
              </span>
              <div className="flex gap-1.5">
                <button disabled={page <= 1 || loading} onClick={() => handlePageChange(1)}
                  className="px-2.5 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg disabled:opacity-30 hover:bg-slate-700">« Primero</button>
                <button disabled={page <= 1 || loading} onClick={() => handlePageChange(page - 1)}
                  className="px-2.5 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg disabled:opacity-30 hover:bg-slate-700">← Anterior</button>
                <button disabled={page >= totalPages || loading} onClick={() => handlePageChange(page + 1)}
                  className="px-2.5 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg disabled:opacity-30 hover:bg-slate-700">Siguiente →</button>
                <button disabled={page >= totalPages || loading} onClick={() => handlePageChange(totalPages)}
                  className="px-2.5 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg disabled:opacity-30 hover:bg-slate-700">Ultimo »</button>
              </div>
            </div>
          )}
        </div>
