// Helpers para subir y visualizar comprobantes de pago
// Se usa tanto en acciones server (supabaseAdmin + service role)
// como en componentes client (validación + compresión).
...
export const SIGNED_URL_TTL_SEG = 600; // 10 min
...
// Redimensiona a DIMENSION_MAX_PX manteniendo aspecto y codifica a JPEG 0.82.
// PDFs pasan sin tocar.
...
// Server-only: sube a Supabase Storage y retorna path + url firmada inicial.
...
// Server-only: genera una signed URL fresca para visualizar un comprobante.
export async function getSignedUrlComprobante(path: string): Promise<string | null> {
  if (!path) return null;
